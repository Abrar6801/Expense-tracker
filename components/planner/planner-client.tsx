...
import {
  Trash2, Plus, TrendingDown, TrendingUp, Wallet, Calculator,
  Users, Banknote, RefreshCw, AlertCircle, CheckCircle2,
} from 'lucide-react'
...
import { useRecurringTemplates, useCreateRecurring, usePostRecurring, useDeleteRecurring } from '@/hooks/use-recurring'
...
interface PlannerClientProps { email?: string }

type PlannerTab = 'overview' | 'budgets' | 'recurring'
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-5 lg:p-6 space-y-4 lg:space-y-6">
        ...
        {tab === 'budgets' && <BudgetsTab month={now.getMonth() + 1} year={now.getFullYear()} />}
...
function OverviewTab({ email: _ }: { email?: string }) {
  ...
  async function handleAddIncome() {
    ...
    if (isNaN(num) || num <= 0) return
    await createIncome.mutateAsync({ name: incomeName.trim(), amount: num, type: incomeType, from: incomeFrom.trim() || undefined })
            ...
            {(['expense', 'income'] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)}
                className={cn(
                  ...
                  activeTab === t
                    ? t === 'expense' ? 'text-rose-400 border-b-2 border-rose-500' : 'text-emerald-400 border-b-2 border-emerald-500'
                    : 'text-white/30 hover:text-white/60'
                )}>
                {t === 'expense' ? 'Planned Expense' : 'Expected Income'}
              </button>
            ))}
          </div>
            ...
            {activeTab === 'expense' ? (
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={accountId} onValueChange={setAccountId}>
                    <SelectTrigger className="bg-white/[0.04] border-white/[0.08] text-white h-10"><SelectValue placeholder="Select account..." /></SelectTrigger>
                    <SelectContent>
                      {accounts?.map(a => (
                ...
                <Button className="w-full h-10 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/20"
                  onClick={handleAddExpense} disabled={!name.trim() || !amount || !category || !accountId || createItem.isPending}>
              ...
              <>
                  ...
                  {(['INCOME_SOURCE', 'MONEY_OWED'] as const).map(t => (
                    <button key={t} type="button" onClick={() => setIncomeType(t)}
                      className={cn(
                        ...
                        incomeType === t ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300' : 'border-white/[0.08] bg-white/[0.02] text-white/40 hover:text-white/70'
                      )}>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input placeholder={incomeType === 'INCOME_SOURCE' ? 'e.g. Freelance Project' : 'e.g. Dinner split'}
                ...
                <Button className="w-full h-10 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/20"
                  onClick={handleAddIncome} disabled={!incomeName.trim() || !incomeAmount || createIncome.isPending}>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="glass-card rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-white/[0.06]">
              ...
              <p className="text-xs text-white/35 mt-0.5">{(items?.length ?? 0) + (incomeItems?.length ?? 0)} items total</p>
            </div>
                    ...
                    <div key={item.id}>
                      <div className="flex items-center gap-3 py-3 group">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <span className="text-sm font-bold font-mono text-rose-400">−{formatCurrency(parseFloat(item.amount), item.account.currency)}</span>
                      ...
                      <div className="flex items-center gap-3 py-3 group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-white/30">
                            {item.type === 'MONEY_OWED' ? 'Money Owed' : 'Income Source'}{item.from ? ` · from ${item.from}` : ''}
                          </p>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          <span className="text-sm font-bold font-mono text-emerald-400">+{formatCurrency(parseFloat(item.amount))}</span>
                          <Button variant="ghost" size="icon"
                            className="h-9 w-9 sm:h-7 sm:w-7 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity text-white/20 hover:text-rose-400 hover:bg-rose-500/10"
...
function BudgetsTab({ month, year }: { month: number; year: number }) {
  ...
  async function handleSave() {
    ...
    await createBudget.mutateAsync({ category, amount: num, month, year })
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="glass-card rounded-2xl p-5 space-y-4 h-fit">
        <div>
          <h3 className="text-sm font-semibold text-white">Set Budget Limit</h3>
          <p className="text-xs text-white/35 mt-0.5">{monthName} {year}</p>
        </div>
        <div className="space-y-1.5">
          <Label className="text-[10px] uppercase tracking-wider text-white/40">Category</Label>
          <Select value={category} onValueChange={setCategory}>
            <SelectTrigger className="bg-white/[0.04] border-white/[0.08] text-white h-10"><SelectValue placeholder="Select category..." /></SelectTrigger>
            <SelectContent>{PRESET_CATEGORIES.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div className="space-y-1.5">
          <Label className="text-[10px] uppercase tracking-wider text-white/40">Monthly Limit</Label>
          <Input type="number" placeholder="0.00" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)}
            className="bg-white/[0.04] border-white/[0.08] focus:border-violet-500/40 text-white placeholder:text-white/20 h-10" />
        </div>
        <Button className="w-full h-10 bg-violet-600/20 hover:bg-violet-600/30 text-violet-300 border border-violet-500/20"
          onClick={handleSave} disabled={!category || !amount || createBudget.isPending}>
          <Plus className="h-4 w-4 mr-2" />Save Budget
        </Button>
      </div>
      ...
      <div className="lg:col-span-2">
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06]">
            <h3 className="text-sm font-semibold text-white">Budget Limits — {monthName} {year}</h3>
            <p className="text-xs text-white/35 mt-0.5">{budgets.length} categories</p>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {isLoading && Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="px-5 py-4 space-y-2">
                <div className="flex justify-between"><Skeleton className="h-3 w-24 bg-white/5" /><Skeleton className="h-3 w-20 bg-white/5" /></div>
            ...
            {!isLoading && budgets.map((budget) => {
              ...
              const barColor = over ? '#fb7185' : budget.percentage > 80 ? '#f59e0b' : '#34d399'
              return (
                <div key={budget.id} className="px-5 py-4 group">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-white/80">{budget.category}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-white/40 font-mono">
                        {formatCurrency(budget.spent)} / {formatCurrency(parseFloat(budget.amount))}
                      </span>
                      <span className={`text-xs font-bold ${over ? 'text-rose-400' : 'text-white/50'}`}>
                        {budget.percentage}%
                      </span>
                      <Button variant="ghost" size="icon"
                  ...
                  {over && (
                    <p className="text-[10px] text-rose-400 mt-1">
                      Over budget by {formatCurrency(budget.spent - parseFloat(budget.amount))}
                    </p>
                  )}
                </div>
              )
...
function RecurringTab() {
  ...
  const [recInterval, setRecInterval] = useState<'weekly' | 'biweekly' | 'monthly' | 'yearly'>('monthly')
  const [nextDue, setNextDue] = useState<Date | undefined>(new Date())

  async function handleCreate() {
    const num = parseFloat(amount)
    if (!accountId || isNaN(num) || num <= 0 || !category || !nextDue) return
    await createRecurring.mutateAsync({
      accountId, type, amount: num, category,
      description: description || undefined, interval: recInterval, nextDue,
    })
    setAmount(''); setCategory(''); setDescription(''); setNextDue(new Date())
  }

  const now = new Date()
  const isDue = (dateStr: string) => new Date(dateStr) <= now
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="glass-card rounded-2xl p-5 space-y-4 h-fit">
        <div>
          <h3 className="text-sm font-semibold text-white">Add Recurring</h3>
          <p className="text-xs text-white/35 mt-0.5">Automatically track regular transactions</p>
        </div>
          ...
          {(['EXPENSE', 'INCOME'] as const).map(t => (
            <button key={t} onClick={() => setType(t)}
              className={cn(
                ...
                type === t
                  ? t === 'EXPENSE' ? 'border-rose-500/40 bg-rose-500/10 text-rose-300' : 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300'
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            <Label className="text-[10px] uppercase tracking-wider text-white/40">Interval</Label>
            <Select value={recInterval} onValueChange={v => setRecInterval(v as typeof recInterval)}>
              <SelectTrigger className="bg-white/[0.04] border-white/[0.08] text-white h-10"><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-1.5">
          ...
          <Input placeholder="e.g. Rent payment" value={description} onChange={e => setDescription(e.target.value)}
            className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-white/20 h-10" />
        </div>

        <div className="space-y-1.5">
          <Label className="text-[10px] uppercase tracking-wider text-white/40">First / Next Due</Label>
          <DatePicker value={nextDue} onChange={setNextDue} allowFuture />
        </div>

        <Button className="w-full h-10 bg-violet-600/20 hover:bg-violet-600/30 text-violet-300 border border-violet-500/20"
          onClick={handleCreate} disabled={!accountId || !amount || !category || !nextDue || createRecurring.isPending}>
      ...
      <div className="lg:col-span-2">
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06]">
            <h3 className="text-sm font-semibold text-white">Recurring Templates</h3>
            <p className="text-xs text-white/35 mt-0.5">{templates.length} active</p>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {isLoading && Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-4">
                <Skeleton className="h-9 w-9 rounded-xl bg-white/5" />
                <div className="flex-1 space-y-2"><Skeleton className="h-3 w-1/3 bg-white/5" /><Skeleton className="h-2.5 w-1/2 bg-white/5" /></div>
                <Skeleton className="h-8 w-16 rounded-lg bg-white/5" />
              </div>
            ))}
            {!isLoading && templates.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                <RefreshCw className="h-8 w-8 text-white/15" />
                <p className="text-sm text-white/30">No recurring templates yet</p>
              </div>
            )}
            {!isLoading && templates.map(t => {
              const due = isDue(t.nextDue)
              const isExpense = t.type === TransactionType.EXPENSE
              return (
                <div key={t.id} className="flex items-center gap-3 px-5 py-4 group">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl"
                    style={{
                      background: isExpense ? 'rgba(251,113,133,0.12)' : 'rgba(52,211,153,0.12)',
                      color: isExpense ? '#fb7185' : '#34d399',
                    }}>
